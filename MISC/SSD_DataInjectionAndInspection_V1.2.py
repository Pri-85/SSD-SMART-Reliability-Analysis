...
# Basic missingness pattern (for MCAR/MAR intuition)
missing_indicators = df[numeric_candidates].isna().astype(int)
missing_corr = missing_indicators.corr()  # kept in memory if you want to inspect later
...
# ------------------------------------------------------------
# Improved Vendor Latency Visualization (Boxplots + Bar Charts)
# ------------------------------------------------------------
...
if vendor_col and lat_col:
    ...
    if not df_vendor_lat.empty:
        ...
        # -----------------------------
        # 2. Bar Chart: Mean Latency by Vendor
        # -----------------------------
        ...
        # -----------------------------
        # 3. Boxplot: Latency by Year (if multiple years exist)
        # -----------------------------
        if df_vendor_lat["year"].nunique() > 1:
            ...
            # -----------------------------
            # 4. Bar Chart: Mean Latency by Year
            # -----------------------------
...
def add_bins(series, bins=4):
    """Safe auto-binning that never crashes."""
    s = series.dropna()
...
# Explicit categorical fields you care about
...
# Any non-numeric column is categorical
all_categorical_cols = [c for c in df.columns if df[c].dtype == "object"]

# Ensure explicit categorical are included even if parsed as numeric (year/month originally)
for c in explicit_categorical:
    if c in df.columns and c not in all_categorical_cols:
        all_categorical_cols.append(c)

# Remove obvious pure ID-like crit if you want, but you chose C2 (so keep all)

# 15.1 Frequency plots for all categoricals
for col in all_categorical_cols:
    ...
    # Plot bar chart (top 50 values to avoid insane axes)
...
# 15.2 Categorical vs numeric boxplots (all categoricals, C2)
for cat_col in all_categorical_cols:
    for num_col in numeric_candidates:
        ...
        # Limit categories to top 20 by frequency to keep plot readable,
        # but still obey C2 (we consider all categoricals; we just group tail into 'Other')
        freq = pair_df[cat_col].value_counts()
        if len(freq) > 20:
            top_categories = freq.index[:19]
            pair_df[cat_col] = np.where(pair_df[cat_col].isin(top_categories),
                                        pair_df[cat_col],
                                        "Other")
