"""
Hypothesis 1: cross_vendor_normalization_experiment.py
...
This script:
- Derives a binary failure label from SMART/degradation indicators.
...
- Evaluates cross-vendor generalization using system_manufacturer as the vendor.
...
class FeatureConfig:
    """
    This configuration is aligned with the actual dataset columns:
    ...
    power_on_hours, controller_busy_time, percentage_used, wear_level_avg,
    wear_level_max, endurance_estimate_remaining, unsafe_shutdowns,
    background_scrub_time_pct, gc_active_time_pct, media_errors,
    error_information_log_entries, bad_block_count_grown,
    pcie_correctable_errors, pcie_uncorrectable_errors, workload_type,
    queue_depth, workload_block_size_kb, system_manufacturer,
    system_configuration, cpu_name, serial_number, system_id,
    system_generation, nand_type_synthetic, year, month,
    host_read_cmds_per_power_cycle.
    """

    # Features that are directly comparable across vendors/systems
    ...
    # Vendor-/drive-specific degradation and error features
    drive_manufacturer_specific_features: List[str] = field(default_factory=lambda: [
        ...
        "unsafe_shutdowns",
        "throttling_events",  # synthetic: derived from composite_temperature_c > 70
    ])

    # Vendor and model identifiers
    drive_manufacturer_col: str = "system_manufacturer"
    drive_model_number_col: str = "model_number"
...
class NormalizationSchema:
    """
    Maps raw vendor-/drive-specific attributes into latent degradation constructs.
    """
...
class CrossVendorNormalizer(BaseEstimator, TransformerMixin):
    """
    Builds latent constructs from vendor-/drive-specific attributes and
    scales them, then concatenates with comparable SMART features.
    """
...
def derive_failure_label(df: pd.DataFrame) -> pd.DataFrame:
    """
    Derive a binary failure label from degradation-related indicators.

    Synthetic throttling_events is defined as: composite_temperature_c > 70.
    """

    df = df.copy()

    # Synthetic throttling indicator based on temperature
    ...
    # Ensure missing degradation columns exist with safe defaults
    for col in [
        "percentage_used",
        "wear_level_max",
        "endurance_estimate_remaining",
...
def prepare_datasets(
    ...
    # Raw feature set: comparable + manufacturer-specific
    ...
    # Normalized feature set: comparable + latent constructs
    normalizer.fit(df)
...
def run_cross_vendor_experiment(df_raw: pd.DataFrame, df_syn: pd.DataFrame) -> None:
    ...
    if feature_cfg.drive_manufacturer_col not in df_syn.columns:
        print("drive_manufacturer_col not found in dataset; skipping cross-vendor generalization.")
        return

    vendors = df_syn[feature_cfg.drive_manufacturer_col].unique().tolist()
    if len(vendors) > 1:
