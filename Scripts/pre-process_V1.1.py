...
# Capacity buckets for normalization
CAPACITY_SKU_BUCKETS = [4, 8, 15, 25, 60, 128, 256]
...
def parse_smart_log(filepath: Path) -> dict:
    ...
    #record["nand_type"] = extract_value(r'NAND Type:\s+(.+)', text)
    ...
    record["workload_type"] = extract_value(r"Workload Type:\s+(.+)", text)
